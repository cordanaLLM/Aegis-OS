...
`planning/components.json` and `planning/roadmap.json` first, run `make readiness`
to find the ready milestones, and work on the highest-ranked ready milestone.
Update a milestone's `state` and `evidence` only with real committed evidence;
`make verify-all` rejects inconsistent blocking states. Load one relevant source
artifact at a time.
