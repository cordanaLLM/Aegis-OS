...
//! `aegis-janus-lifecycle`, `aegis-hestia` and `aegis-tellus`.
//!
...
//! a planted `pub static UNTESTED_REGISTER`. This crate carries several
//! all-public-field types -- the checkpoint draft, the metric set, the
//! promotion trigger, the recorded claim and the decision record -- so the
//! field arm carries more weight here, not less.
//!
...
///
/// The figure is measured, not guessed: 237 names are collected from this
/// crate's `src/` on 2026-09-13, of which [`FIELD_ARM_NAMES`] come from the
/// public-field and `pub static` arm of [`declared_name`] that the M15
/// verification found missing. The floor sits nine below the total so that
/// removing one item on purpose does not fail the gate, while dropping the
/// field arm -- which costs twenty -- does. Removing public surface on purpose
/// means lowering the floor on purpose.
const MINIMUM_SURFACE: usize = 228;

/// How many of the collected names only the M15 arm reads.
///
/// Measured on 2026-09-13 by running the same walk twice: 237 names with the
/// arm and 217 without it. `the_field_arm_carries_the_floor` asserts the
/// difference, so the figure cannot drift out of agreement with the code the
/// way a number stated only in prose can. Every one of the 20 is a public
...
fn the_scanner_reads_public_fields_and_statics_and_not_private_ones() {
    for (line, expected) in [
        ...
        ("pub candidate: CandidateId,", "candidate"),
        ("pub retention: NullModelRetention,", "retention"),
        ("pub cleared: u8,", "cleared"),
        ("pub const fn tag(self) -> u8 {", "tag"),
    ] {
        assert_eq!(
    ...
    for ignored in [
        ...
        "pub use crate::machine::Lifecycle;",
        "pub mod contracts;",
        "let pub_like = 1;",
        "",
    ] {
...
fn the_identifier_matcher_is_exact_at_both_ends() {
    assert!(contains_identifier("AthenaError::Terminal", "Terminal"));
    assert!(!contains_identifier("MIN_PASSING_GATES", "PASSING_GATES"));
    assert!(!contains_identifier("is_superior_to", "is_superior"));
    assert!(!contains_identifier("outer_Lifecycle", "Lifecycle"));
    assert!(!contains_identifier("Lifecycle2", "Lifecycle"));
    assert!(contains_identifier("Lifecycle", "Lifecycle"));
    assert!(!contains_identifier("", "Lifecycle"));
}
