...
//! field types -- every plugin row, every descriptor, every register record --
...
fn the_scanner_reads_public_fields_and_statics_and_not_private_ones() {
    for (line, expected) in [
        ...
        ("pub format: PluginFormat,", "format"),
...
fn the_identifier_matcher_is_exact_at_both_ends() {
    assert!(contains_identifier(
        "CalliopeError::PluginTableFull",
        "PluginTableFull"
    ));
    assert!(!contains_identifier("MAX_PLUGIN_SLOTS", "PLUGIN_SLOTS"));
    assert!(!contains_identifier("stride_bytes_of", "stride_bytes"));
    assert!(!contains_identifier("outer_PluginHost", "PluginHost"));
    assert!(!contains_identifier("PluginHost2", "PluginHost"));
    assert!(contains_identifier("PluginHost", "PluginHost"));
    assert!(!contains_identifier("", "PluginHost"));
}
