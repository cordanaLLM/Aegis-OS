...
//!
//! The crate documentation says it opens nothing and measures nothing. That
...
//! below are the ones a `PipeWire` connection, a DMA-BUF export, a plugin
//! bridge, a resource-limit call, a socket of any family or a clock read would
...
/// The identifiers a graph, an export, a bridge, a limit call, a socket or a
...
///
/// `pipewiresrc` and `SPA_DATA_DmaBuf` are the recorded transport string of
/// [`EdgeId::ShareDmaBufStream`](aegis_calliope::EdgeId), which the graph of
/// record states and this crate carries as data. A sweep that watched them
/// would fail on the field it is supposed to protect; what it watches instead
/// is the machinery that would open one.
const NOT_WATCHED: [&str; 2] = ["pipewiresrc", "SPA_DATA_DmaBuf"];
...
fn the_sweep_reads_the_whole_crate() -> Fallible {
    ...
    for expected in ["lib.rs", "plugin.rs", "dmabuf.rs", "rtprio.rs", "sync.rs"] {
        assert!(
...
fn the_sweep_detects_a_planted_identifier() {
    ...
    let second = "    let stream = pipewire::stream::Stream::new(&core)?;";
    assert_eq!(
        common::scan_text("planted.rs", second, &WATCHED),
        vec!["planted.rs:1: pipewire::".to_owned()]
    );
    let third = "    unsafe { libc::setrlimit(RLIMIT_RTPRIO, &limit) };";
    let hits = common::scan_text("planted.rs", third, &WATCHED);
    assert!(hits.contains(&"planted.rs:1: libc".to_owned()));
    assert!(hits.contains(&"planted.rs:1: setrlimit".to_owned()));
...
fn a_comment_naming_an_identifier_is_not_a_finding() {
    let comment = "//! no libc, no setrlimit, no Instant:: and no pipewire:: connection";
...
/// Boundary: the recorded transport strings are deliberately outside the list,
/// and the library does carry them -- so watching them would fail the gate on
/// the very data the graph of record requires.
#[test]
fn the_recorded_transport_strings_are_deliberately_not_watched() -> Fallible {
    ...
    assert!(
        !found.is_empty(),
        "the recorded transport strings must appear in library code as EdgeId data"
    );
    Ok(())
}
