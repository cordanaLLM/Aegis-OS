...
//! The generalised public-surface sweep, carried over from `aegis-justitia`.
//!
...
//! a trait item, an associated or free constant -- must appear as a whole
//! identifier in at least one integration test. A public item with no test
//! reference is either untested surface or surface that should not exist; both
//! are findings, and this file turns either into a failed gate.
//!
...
//! test files: `repart_definitions.rs`, `transfer_definitions.rs`,
//! `shipped_definitions.rs`, `sizes.rs` and `unit_file.rs`. This sweep only
//! guarantees that none of them is forgotten.
//!
...
fn the_identifier_matcher_is_exact_at_both_ends() {
    assert!(contains_identifier(
        "Finding::EspUnbounded { .. }",
        "EspUnbounded"
    ));
    assert!(!contains_identifier("MAX_ENTRIES", "ENTRIES"));
    assert!(!contains_identifier("parse_size_of", "parse_size"));
    assert!(!contains_identifier("outer_UnitFile", "UnitFile"));
    assert!(!contains_identifier("UnitFile2", "UnitFile"));
    assert!(contains_identifier("UnitFile", "UnitFile"));
    assert!(!contains_identifier("", "UnitFile"));
}
