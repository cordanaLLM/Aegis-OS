...
//! field types -- every process row, every payload, every register record --
...
fn the_scanner_reads_public_fields_and_statics_and_not_private_ones() {
    for (line, expected) in [
        ...
        ("pub tier: ProcessTier,", "tier"),
        ("pub declared_vram_mib: u32,", "declared_vram_mib"),
        ("pub proposed_version: &'static str,", "proposed_version"),
...
fn the_identifier_matcher_is_exact_at_both_ends() {
    ...
    assert!(!contains_identifier(
        "declared_vram_mib_of",
        "declared_vram_mib"
    ));
    assert!(!contains_identifier(
        "outer_ResourceBroker",
        "ResourceBroker"
    ));
    assert!(!contains_identifier("ResourceBroker2", "ResourceBroker"));
    assert!(contains_identifier("ResourceBroker", "ResourceBroker"));
    assert!(!contains_identifier("", "ResourceBroker"));
}
