...
//! field types -- every sandbox row, every payload, every register record --
...
const MINIMUM_SURFACE: usize = 184;
...
const FIELD_ARM_NAMES: usize = 37;
...
fn the_scanner_reads_public_surface_and_not_private_helpers() {
    ...
    for expected in [
        ...
        "CapsuleSlot",
        "MAX_MICROVMS",
        "SCAFFOLD_BOOT_TIME_MS",
        "MicroVmTableFull",
        "D06_WASM_RUNTIME",
        "P10_RECORDED_CLAIMS",
        "CapsuleRequest",
        "declared_boot_time_ms",
        "correlation_id",
        "sha256_prefix",
        "rejected",
        "probed_on",
...
fn the_scanner_reads_public_fields_and_statics_and_not_private_ones() {
    for (line, expected) in [
        ...
        ("pub vmm: VmmIdentity,", "vmm"),
        ("pub vsock_cid: VsockCid,", "vsock_cid"),
    ...
    for ignored in [
        ...
        "pub use crate::capsule::CapsuleSlot;",
        "pub mod contracts;",
        "let pub_like = 1;",
        "",
    ] {
...
fn the_identifier_matcher_is_exact_at_both_ends() {
    assert!(contains_identifier(
        "VestaError::MicroVmTableFull",
        "MicroVmTableFull"
    ));
    assert!(!contains_identifier("MAX_MICROVMS", "MICROVMS"));
    assert!(!contains_identifier("vsock_cid_of", "vsock_cid"));
    assert!(!contains_identifier("outer_CapsuleSlot", "CapsuleSlot"));
    assert!(!contains_identifier("CapsuleSlot2", "CapsuleSlot"));
    assert!(contains_identifier("CapsuleSlot", "CapsuleSlot"));
    assert!(!contains_identifier("", "CapsuleSlot"));
}
