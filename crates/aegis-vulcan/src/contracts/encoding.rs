...
//! The field encoding both M17 descriptors share.
//!
...
//!
//! * **Validation happens during decoding.** A correlation identifier is
//!   parsed through the same constructor the library already uses, so an
//!   over-long or ill-formed field is refused by the decoder rather than
//!   accepted and checked afterwards. Every other field is a newtype over an
//!   integer wired to its own `TryFrom`, which is why a block count of 8193
//!   does not decode.
//! * **Every value lands in a fixed inline slot.** Nothing in this file owns
//!   heap, which is why both descriptor types are `Copy`. What this does not
